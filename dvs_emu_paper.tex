...
Since we will use discrete time steps, we divide time interval $T$ into $N_{b}$ time bins of width $W_{b}$. Each bin represents a brightness change, in either a linear or logarithmic scale. We will first treat the case in which pixels are only allowed to emit one spike per frame. If $N_{b} = T$ then we can calculate the appropriate time bin $C_{b}$ in linear scale with
\begin{equation}
\label{eq:linear_bin_calc}
C_{b} = N_{b} - \mathrm{min}\left( N_{b}, \;\; N_{H} \right) 
\end{equation}

The maximum brightness change we can express with one spike is $N_{b}\cdot H$, Figure~\ref{fig:linear_time_all} shows the value-to-spike time relation. Notice that in our proposal earlier spikes represent larger changes in intensity. The main advantage of this encoding is that a single spike could represent multiple rate-based spikes, though the encoded values are limited by time resolution and the frame rate of the camera.

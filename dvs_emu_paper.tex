...
\end{equation}
where, in this encoding, $N_{H}$ is the number of spikes needed to represent the change in brightness $\Delta B$ in terms of the threshold $H$. Notice that the maximum time ($T$) to transmit all the spikes for one frame is bound by the frame rate of the camera~$(r)$. If the time resolution of spike events is one millisecond, then $T = 1000/r$. At this stage we model a perfect receiver, so the update rule for the reference is
\begin{equation}
  \label{eq:ref_update}
